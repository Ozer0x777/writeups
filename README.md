...
| [`lazarus-contagious-interview-pspn/`](lazarus-contagious-interview-pspn/) | Lazarus Contagious Interview (faux projet de test technique GitHub) | Dropper JS caché dans un dashboard web3 crédible, déclenché au premier `npm start` ; stage 2 chiffré (AES-256-CBC/scrypt) récupéré en direct auprès du C2 réel et déchiffré, révélant 4 sous-modules (RAT Socket.IO + SSH interactif, voleur de données navigateur, moissonneur de fichiers, espion de presse-papiers), tout rejoué dans un bac à sable Node.js sans exécution réelle. |
| [`scorched-earth-ausfc/`](scorched-earth-ausfc/) | Ransomware Prinz Eugen (module de chiffrement, Go) | Reverse d'un module de chiffrement Go non strippé (format `chv1`, Argon2id → SHA-256 → HKDF-SHA256, ChaCha20-Poly1305) : mot de passe généré à la compilation retrouvé codé en dur, déchiffrement réel confirmé par ré-implémentation Python. Attribution à la famille Prinz Eugen par recoupement avec une analyse externe (ThreatDown), qui ne rapporte pas ce défaut sur son propre échantillon. |
